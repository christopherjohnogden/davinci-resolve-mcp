...
def test_project_settings() -> bool:
    ...
    result1 = send_request("mcp_davinci_resolve_set_project_setting", 
                          {"setting_name": "timelineFrameRate", "setting_value": 24})
    
    # Test with string value
    logger.info("Testing string parameter...")
    result2 = send_request("mcp_davinci_resolve_set_project_setting", 
                          {"setting_name": "timelineFrameRate", "setting_value": "24"})
    
    # Test with float value
    logger.info("Testing float parameter...")
    result3 = send_request("mcp_davinci_resolve_set_project_setting", 
...
def test_color_page_operations() -> bool:
    ...
    result2 = send_request("mcp_davinci_resolve_add_node", 
                         {"node_type": "serial", "label": "AutoTest"})
    
    # Try setting color wheel parameter
    result3 = send_request("mcp_davinci_resolve_set_color_wheel_param", 
    ...
    if success1 and (success2 or auto_select_working):
        logger.info("✅ Color page operations are working or properly reporting selection issues")
        return True
    else:
        logger.error(f"❌ Color page operations test failed")
        logger.error(f"    Switch to color page: {'✅ Passed' if success1 else '❌ Failed'}")
        logger.error(f"    Add node: {'✅ Passed' if success2 else '❌ Failed but proper error' if auto_select_working else '❌ Failed'}")
...
def test_render_queue_operations() -> bool:
    ...
    result2 = send_request("mcp_davinci_resolve_clear_render_queue", {"random_string": "test"})
    
    # Try adding a timeline to the render queue
    time.sleep(1)  # Give it a moment to switch pages
    result3 = send_request("mcp_davinci_resolve_add_to_render_queue", 
                         {"preset_name": "YouTube 1080p", "timeline_name": None, "use_in_out_range": False})
    ...
    if not success3:
        ...
        if "ensure_render_settings" in error_msg or "validate_render_preset" in error_msg:
    ...
    if success1 and success2 and (success3 or helper_working):
        ...
        logger.error(f"    Switch to deliver page: {'✅ Passed' if success1 else '❌ Failed'}")
        logger.error(f"    Clear render queue: {'✅ Passed' if success2 else '❌ Failed'}")
...
def test_error_handling_with_empty_timeline() -> bool:
    ...
    create_result = send_request("mcp_davinci_resolve_create_timeline", 
                               {"name": empty_timeline_name})
    
    # Set it as current
    set_result = send_request("mcp_davinci_resolve_set_current_timeline", 
                            {"name": empty_timeline_name})
    ...
    result2 = send_request("mcp_davinci_resolve_add_node", 
                         {"node_type": "serial", "label": "EmptyTest"})
    
    success1 = "error" not in result1 or not result1.get("error")
    
    # Check for improved error handling
    improved_error = False
    expected_phrases = ["no clip", "empty timeline", "select clip", "ensure_clip_selected"]
    ...
    if "error" in result2 and result2.get("error"):
        ...
        for phrase in expected_phrases:
            if phrase in error_msg:
                improved_error = True
                logger.info(f"✅ Proper error handling detected: '{phrase}' found in error message")
                break
    
    # Clean up - delete the test timeline
    delete_result = send_request("mcp_davinci_resolve_delete_timeline", 
                               {"name": empty_timeline_name})
...
def test_parameter_validation() -> bool:
    ...
    tests = [
        ...
        {"type": "string boolean", "value": "true", "setting": "timelineResolutionWidth"}
    ]
    ...
    for test in tests:
        ...
        result = send_request("mcp_davinci_resolve_set_project_setting", 
        ...
        if success:
            ...
            logger.error(f"❌ {test['type']} parameter rejected: {result.get('error', 'Unknown error')}")
