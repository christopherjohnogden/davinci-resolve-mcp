...
[![Version](https://img.shields.io/badge/version-2.30.10-blue.svg)](https://github.com/samuelgursky/davinci-resolve-mcp/releases)
...
- Optional remote visual analysis: pass `analysis_backend="runpod"` to `analyze_clip_visual` or `media_analysis(action="analyze_clip_visual", ...)`. Local analysis remains the default. Configure `RUNPOD_ANALYSIS_ENDPOINT_ID` and `RUNPOD_API_KEY`; for network-volume staging also set `RUNPOD_NETWORK_VOLUME_ID`, the datacenter/endpoint, and RunPod S3 credentials. If no network volume is configured, provide `runpod_file_url` or a `RUNPOD_MEDIA_LOCAL_PREFIX` / `RUNPOD_MEDIA_URL_PREFIX` mapping so the worker can read a source-safe copy of the media. Use `media_analysis(action="runpod_visual_submit_batch")` to submit one remote job per clip for multi-worker ingest, then `runpod_visual_status` to poll and commit sidecars/metadata. Staged media defaults to a 14-day retention policy via `RUNPOD_STAGING_RETENTION_DAYS=14`; actual deletion is enforced by the staging storage lifecycle/cleanup. `examples/runpod_visual_worker.py` is the matching serverless worker entrypoint.
- RunPod analysis can opt into analysis-only proxies with `runpod_use_proxy=true` and `runpod_proxy_width=960`. The proxy is written under `~/Resolve_Analysis`, uploaded instead of the camera original, and never relinked into Resolve; returned frame numbers and metadata still attach to the original media-pool clip.
- The server loads repo-local `.env` and `.env.local` files automatically for local secrets and RunPod settings. Existing shell exports, MCP client env blocks, and `RESOLVE_MCP_ENV_FILE=/path/to/file.env` take priority.
